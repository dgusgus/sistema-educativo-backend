// prisma/seed.ts
// Seed completo actualizado — incluye Director y Secretaria con perfiles propios
...
async function main() {
  ...
  const [trim1, trim2, trim3] = await Promise.all([
    ...
    prisma.trimestre.upsert({
      ...
      create: {
        ...
        gestionId: gestion.id,
        fechaInicio: new Date('2025-08-04'),
        fechaFin:    new Date('2025-11-28'),
        cerrado: false,
      },
    }),
  ...
  // ══════════════════════════════════════
  // DIRECTOR — perfil completo + cuenta vinculada
  // ══════════════════════════════════════
  const director = await prisma.director.upsert({
    ...
    create: {
      ...
      activo:    true,
      usuarioId: uDir.id,
      gestionId: gestion.id,   // ← a cargo de la gestión 2025
    },
  })
  console.log(`✓ Director: ${director.nombre} ${director.apellido}`)

  // ══════════════════════════════════════
  // SECRETARIA — perfil completo + cuenta vinculada
  // ══════════════════════════════════════
  ...
  await vincular(tut1.id, est4.id)   // Rosa → Miguel (tiene 2 hijos)
  await vincular(tut2.id, est2.id)   // Miguel → Pedro
  await vincular(tut3.id, est3.id)   // Carmen → Lucía
  await vincular(tut3.id, est5.id)   // Carmen → Valeria (tiene 2 hijos)
  ...
  // 1ro Sec A
  const dmc1 = await dmc(doc1.id, mMAT.id, c1A.id)  // Mamani → MAT
  const dmc2 = await dmc(doc2.id, mLEN.id, c1A.id)  // Quispe → LEN
  const dmc3 = await dmc(doc3.id, mING.id, c1A.id)  // Flores → ING
  const dmc4 = await dmc(doc3.id, mEFI.id, c1A.id)  // Flores → EFI
  const dmc5 = await dmc(doc4.id, mCNA.id, c1A.id)  // Condori → CNA
  const dmc6 = await dmc(doc4.id, mCSO.id, c1A.id)  // Condori → CSO
  // 1ro Sec B y 2do Sec A
  ...
  // MAT — estados variados para probar alertas
  const estadosMat = [
  ...
  for (let d = 0; d < fechas.length; d++) {
    for (let e = 0; e < inscC1A.length; e++) {
      await prisma.asistencia.upsert({
        where: {
          inscripcionId_docenteMateriaCursoId_fecha: {
            inscripcionId:         inscC1A[e].id,
            docenteMateriaCursoId: dmc1.id,
            fecha:                 new Date(fechas[d]),
          },
        },
        update: {},
        create: {
          ...
          docenteMateriaCursoId: dmc1.id,
          fecha:                 new Date(fechas[d]),
          estado:                estadosMat[d][e] as any,
        },
      })
    }
  }
  console.log('✓ Asistencia T1 — MAT (Pedro: 60%, Miguel: 40% crítico)')

  // LEN — mayormente presentes
  ...
  console.log('✓ Asistencia T1 — LEN')

  // ══════════════════════════════════════
  // CALIFICACIONES T1 (trimestre cerrado)
  // ══════════════════════════════════════
  ...
  console.log('✓ Calificaciones T1 (Miguel tiene notas < 51 en varias materias)')

  // ══════════════════════════════════════
  // RESÚMENES DE ASISTENCIA T1
  // ══════════════════════════════════════
  const resumenes = [
    // Ana — excelente asistencia
    ...
    // Pedro — asistencia crítica en MAT
    ...
    // Miguel — asistencia crítica (caso más grave)
    ...
    await prisma.resumenAsistencia.upsert({
      where: {
        inscripcionId_docenteMateriaCursoId_trimestreId: {
          ...
          docenteMateriaCursoId: r.docenteMateriaCursoId,
  ...
  // CONCEPTOS DE PAGO
  // ══════════════════════════════════════
  const cp1 = await prisma.conceptoPago.create({
    data: { nombre: 'Matrícula 2025',             descripcion: 'Pago de matrícula gestión 2025.',             monto: 150, obligatorio: true,  gestionId: gestion.id },
  }).catch(async () => (await prisma.conceptoPago.findFirst({ where: { nombre: 'Matrícula 2025',             gestionId: gestion.id } }))!)
  const cp2 = await prisma.conceptoPago.create({
    data: { nombre: 'Material Didáctico',          descripcion: 'Contribución para material y fotocopias.',    monto:  80, obligatorio: true,  gestionId: gestion.id },
  }).catch(async () => (await prisma.conceptoPago.findFirst({ where: { nombre: 'Material Didáctico',          gestionId: gestion.id } }))!)
  const cp3 = await prisma.conceptoPago.create({
    data: { nombre: 'Mantenimiento Infraestructura', descripcion: 'Contribución voluntaria de mantenimiento.', monto:  50, obligatorio: false, gestionId: gestion.id },
  }).catch(async () => (await prisma.conceptoPago.findFirst({ where: { nombre: 'Mantenimiento Infraestructura', gestionId: gestion.id } }))!)
  console.log('✓ 3 conceptos de pago')

  // ══════════════════════════════════════
  // PAGOS — escenarios variados para pruebas
  // ══════════════════════════════════════
  const pago = async (
  ...
  // Pedro — solo matrícula (falta material)
  await pago(insc2.id, cp1.id, 150, 'TRANSFERENCIA', 'REC-2025-0004')

  // Lucía — matrícula y material, sin mantenimiento
  await pago(insc3.id, cp1.id, 150, 'EFECTIVO',      'REC-2025-0005')
  await pago(insc3.id, cp2.id,  80, 'QR',            'REC-2025-0006')

  // Miguel — pago anulado + repago (escenario de corrección)
  await pago(insc4.id, cp1.id, 150, 'EFECTIVO',      'REC-2025-0007', 'ANULADO')
  await pago(insc4.id, cp1.id, 150, 'EFECTIVO',      'REC-2025-0008')
  await pago(insc4.id, cp2.id,  80, 'QR',            'REC-2025-0009')

  // Valeria — sin ningún pago (para probar estado pendiente)
  console.log('✓ Pagos (Ana: todo, Pedro: solo matrícula, Valeria: sin pagar)')
  ...
  const actividades = [
    ...
    { fecha: '2025-02-10', tema: 'Números decimales',        descripcion: 'Representación decimal y operaciones.',    tarea: 'Ejercicios pág. 22-23'      },
    { fecha: '2025-02-17', tema: 'Porcentajes',              descripcion: 'Cálculo de % en problemas cotidianos.',    tarea: '10 problemas de aplicación'  },
  ...
  for (const a of actividades) {
    try {
      await prisma.actividad.create({
        data: { docenteMateriaCursoId: dmc1.id, fecha: new Date(a.fecha), tema: a.tema, descripcion: a.descripcion, tareaAsignada: a.tarea },
      })
    } catch { /* ignora duplicados */ }
  }
  console.log('✓ 5 actividades del docente Mamani')
  ...
  console.log('│ est_ana      │ est1234     │ ESTUDIANTE  │ Ana Condori Mamani       │')
  console.log('│ est_pedro    │ est1234     │ ESTUDIANTE  │ Pedro Huanca Quispe      │')
  console.log('│ est_lucia    │ est1234     │ ESTUDIANTE  │ Lucía Tarqui Flores      │')
  console.log('│ est_miguel   │ est1234     │ ESTUDIANTE  │ Miguel Mamani Choque     │')
  console.log('│ est_valeria  │ est1234     │ ESTUDIANTE  │ Valeria Quispe Tarqui    │')
  console.log('│ tut_rosa     │ tut1234     │ TUTOR       │ Rosa Condori (Ana+Miguel)│')
  console.log('│ tut_miguel   │ tut1234     │ TUTOR       │ Miguel Huanca (Pedro)    │')
  console.log('│ tut_carmen   │ tut1234     │ TUTOR       │ Carmen Tarqui (Lucía+Val)│')
  console.log('└──────────────┴─────────────┴─────────────┴──────────────────────────┘')
  console.log('\nCASOS DE PRUEBA INTERESANTES:')
  console.log('  Pedro   → asistencia MAT 60% → aparece en alertas')
  console.log('  Miguel  → asistencia MAT 40% + notas < 51 → caso crítico')
  console.log('  Miguel  → pago ANULADO + repago → probar historial')
  console.log('  Valeria → sin ningún pago → estado pendiente')
  console.log('  T1      → cerrado → editar notas retorna 403')
  console.log('  Director Roberto Vargas → asignado a gestión 2025 → aparece en boletines')
}
